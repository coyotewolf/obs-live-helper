...
- [ ] Fix 加入佇列審核 bug. 問題複現-未開放點歌直接加入佇列權限時，點歌並點擊加入佇列進入審核清單後，若主播同意加入佇列，會造成整個原先的播放清單消失，Up Next顯示歌曲全部變成帶播歌曲。
- [ ] Fix LRCLib lyrics-not-found timeout handling 查詢GPT紀錄 bug.
