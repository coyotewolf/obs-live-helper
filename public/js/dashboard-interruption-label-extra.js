...
(function dashboardInterruptionHelpers(){
  const DATA_URL = '/song-requests.json';
  ...
  function ensureStyles(){
    ...
    style.textContent = `
      ...
      .approvedInterruptionItem{display:grid;grid-template-columns:58px minmax(0,1fr) auto;gap:12px;align-items:center;padding:12px;border:1px solid rgba(255,255,255,.11);border-radius:18px;background:rgba(255,255,255,.045)}
      ...
      @media(max-width:760px){.approvedInterruptionItem{grid-template-columns:50px minmax(0,1fr)}.approvedInterruptionItem img{width:50px;height:50px}.approvedInterruptionActions{grid-column:1/-1}.approvedInterruptionBadge{justify-content:flex-start}}
    `;
    ...
    panel.innerHTML = `
      ...
      <div class="dashboardAccordionBody">
        <div class="titleRow compactTitleRow">
          <p class="introText">這裡列出主播已同意的插播請求。系統不會自動播放，請主播自行到 Spotify 搜尋或開啟歌曲播放。</p>
          <div class="btnRow compact"><button id="reloadApprovedInterruptionBtn" class="btnGhost" type="button">重新整理</button></div>
        </div>
  ...
  function renderApprovedInterruptions(items = []){
    ...
    list.innerHTML = items.map(item => {
      ...
      return `
        <article class="approvedInterruptionItem">
          ...
          <div class="approvedInterruptionActions">
            ...
            <span class="approvedInterruptionBadge">已同意插播</span>
          </div>
        </article>`;
    }).join('');
  }
  ...
  async function loadApprovedInterruptions(){
    ...
    try {
      ...
      renderApprovedInterruptions(items
